...
After the WordPress page is pasted and published, run `node wordpress-smoke.js --url "https://ncsy.org/ncsy-wrapped/?chapter=baltimore"`. This live shell check confirms the WordPress page has `#jsu-wrapped`, hosted or inline widget CSS/JS, data/config URLs, generated share-page base, CTA target or href, a matching Gravity Forms/context panel when `data-cta-target` is used, a privacy/cookie affordance, paired JSU/NCSY Wrapped OG/Twitter title metadata, meta description, `og:type`, `og:site_name`, `og:description`, `twitter:description`, canonical/social URLs, `og:image`, `twitter:image`, `og:image:secure_url`, `og:image:alt`, and `twitter:image:alt`, and no visible `undefined`, `null`, or `NaN` placeholder text. It is not part of `node check-production.js` because drafts, unpublished pages, and WordPress auth state vary by environment.
