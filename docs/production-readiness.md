...
After pushing, run `node hosted-smoke.js` to fetch the live GitHub Pages entry page, builder noindex guard, CTA prefill noindex guard, widget JS/CSS, builder script, chapter/config JSON, Baltimore static share page metadata, and the social preview image as a 1200x630 `image/png` response. This command is intentionally separate from `node check-production.js` because it needs network access and can fail while Pages is still deploying. Use `node hosted-smoke.js --base https://example.org/wrapped/` for forks, preview hosts, or a WordPress-hosted copy.
