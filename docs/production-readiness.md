...
After pushing, run `node hosted-smoke.js` to fetch the live GitHub Pages entry page, builder noindex guard, CTA form prefill noindex guard, CTA link prefill noindex guard, widget JS/CSS, builder script, WordPress inline embed, chapter/config JSON, teen proof-of-concept JSON, Baltimore static share page metadata, and the social preview image as a 1200x630 `image/png` response. This command is intentionally separate from `node check-production.js` because it needs network access and can fail while Pages is still deploying. Use `node hosted-smoke.js --base https://example.org/wrapped/` for forks, preview hosts, or a WordPress-hosted copy.

For a local browser render check, run `node render-smoke.js --skip-if-missing`. It serves the static files locally, launches an installed Chrome or Edge in headless mode when available, and verifies the picker, Baltimore story, CTA form prefill, CTA link prefill, and builder render real DOM in mobile and desktop viewports. It also exercises a noindex CTA form prefill smoke page so the final-card CTA has to open a Gravity-Forms-style panel and populate chapter context, plus a noindex CTA link prefill smoke page so the final-card CTA has to navigate to a Gravity-Forms-style target URL with short `wrapped_*` params. `node check-production.js` runs the same command with `--skip-if-missing`, preserving the no-dependency workflow on machines without a browser.
