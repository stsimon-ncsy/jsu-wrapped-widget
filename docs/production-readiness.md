...
After pushing, run `node hosted-smoke.js` to fetch the live GitHub Pages entry page, builder noindex guard, CTA form prefill noindex guard, CTA link prefill noindex guard, analytics smoke noindex guard, layout smoke noindex guard, widget JS/CSS, builder script, WordPress inline embed, chapter/config JSON, teen proof-of-concept JSON, Baltimore static share page metadata, and the social preview image as a 1200x630 `image/png` response. For the default GitHub Pages host, the smoke also sends an `https://ncsy.org` origin and confirms the JSON assets include `Access-Control-Allow-Origin`, so a WordPress-hosted public page can fetch GitHub-hosted data/config in the browser. This command is intentionally separate from `node check-production.js` because it needs network access and can fail while Pages is still deploying. Use `node hosted-smoke.js --base https://example.org/wrapped/ --require-cors --origin https://ncsy.org` for another cross-origin static asset host, or `node hosted-smoke.js --base https://example.org/wrapped/ --skip-cors` for a same-origin WordPress-hosted copy.

Run `node launch-audit.js` when you want one maintainer-facing snapshot of the current launch gates. It reports the latest commit, working-tree state, automated local commands, post-deploy/live smoke commands, and the manual external confirmations that cannot be proved from the repo alone: GTM Preview/GA4 DebugView for `G-Y3LLF5KQ23`, a real Gravity Forms test submission, mobile and desktop visual review, social-preview debugger review, and staff ownership for follow-up.

After the WordPress page is pasted and published, run `node wordpress-smoke.js --url "https://ncsy.org/ncsy-wrapped/?chapter=baltimore"`. This live shell check confirms the WordPress page has `#jsu-wrapped`, hosted or inline widget CSS/JS, data/config URLs, generated share-page base, no-header host GTM/Google tag plumbing when the inline shell is used, a static first-paint loading shell, CTA target or href, a matching Gravity Forms/context panel when `data-cta-target` is used, a privacy/cookie affordance, crawler-readable JSU/NCSY Wrapped title, meta description, `og:site_name`, canonical/Open Graph URL, campaign image metadata, and no visible `undefined`, `null`, or `NaN` placeholder text. Yoast/Open Graph output is accepted as the primary social preview source; Twitter-specific tags such as `twitter:image:alt`, secure image duplicates, and image alt fields remain recommended but are not required when the Open Graph preview is complete. It is not part of `node check-production.js` because drafts, unpublished pages, and WordPress auth state vary by environment.
...
Hosted preview pages, the CTA prefill smoke page, the CTA link smoke page, the analytics smoke page, the layout smoke page, the WordPress inline embed, the compact Brizy hosted embed, the WordPress smoke validator, the WordPress launch packet, the builder JSON fetches, and the pasteable README snippet use a shared static asset cache token, currently `jsuw-prod-20260609a`, on JS/CSS/JSON references. When hosted assets or GitHub Pages JSON/config should force-refresh for staff or public reviewers, run `node bump-cache-token.js jsuw-prod-YYYYMMDDx`, then run `node check-production.js`. The helper updates `index.html`, `embed-example.html`, `builder.html`, `cta-prefill-smoke.html`, `cta-link-smoke.html`, `analytics-smoke.html`, `layout-smoke.html`, `wrapped-builder.js`, `wordpress-brizy-hosted-embed.html`, `wordpress-inline-embed.html`, `wordpress-smoke.js`, `README.md`, `docs/production-readiness.md`, `docs/wordpress-launch-packet.md`, and `qa-smoke.js`.
...
That title is applied to `document.title`, `og:title`, and `twitter:title`. The WordPress page should also expose a plain meta description, `og:description`, and `twitter:description` with JSU/NCSY Wrapped copy for the active chapter, region, or program so crawlers do not fall back to footer links such as privacy or cookie text. The in-story logo and card branding can still use JSU or NCSY based on the record/config logo setting.
...
The hosted entry pages and generated share pages include crawler-readable Open Graph/Twitter tags, social descriptions, site name, image dimensions, and social image alt text. Generated share pages immediately redirect human visitors back to the interactive `?chapter=`, `?scope=region&region=`, or `?scope=program&program=` story. The JavaScript redirect preserves supported query params such as `variant`, `program`, `campaign`, `autoplay`, and `duration`, so donor/custom share links still land on the intended experience. Use `data-share-base="./share/"` for same-site hosting, or point WordPress embeds at the GitHub Pages share folder:
...
<div
  id="jsu-wrapped"
  data-year="2026"
  data-source="https://stsimon-ncsy.github.io/jsu-wrapped-widget/sample-wrapped-2026.json?v=jsuw-prod-20260609a"
  data-config-source="https://stsimon-ncsy.github.io/jsu-wrapped-widget/wrapped-config-2026.json?v=jsuw-prod-20260609a"
  data-share-base="https://stsimon-ncsy.github.io/jsu-wrapped-widget/share/"
