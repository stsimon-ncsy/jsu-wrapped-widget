...
- Confirm the shared cache token is current across README snippets, `index.html`, `embed-example.html`, `builder.html`, `wrapped-builder.js`, `wordpress-inline-embed.html`, and hosted JSON/CSS/JS URLs.
...
- If using the staff builder pilot form, confirm the `wrapped_submission` textarea supports dynamic population and accepts a pasted JSON fallback.
