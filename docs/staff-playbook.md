...
For pilot staff who should not edit GitHub directly, ask them to fill in the builder's required submission info fields, then use **Open review form** when a `review_url` is configured. If no review form is configured, use **Open email draft** as the fallback, or use **Copy submission** / **Download submission** for Slack, Teams, locked-down browsers, or oversized packets. That review payload is intentionally smaller than the full config export: it includes the submitter name/email/note, the active chapter, region, or program scope, the variant if one is selected, a change summary, and the exact `merge_path` where the reviewed patch belongs in `wrapped-config-2026.json`.
...
Use `docs/staff-submission-intake.md` when you need the maintainer-facing setup: Gravity Forms fields, dynamic population names, a staff message template, and the local review/merge commands.

To create that link without hand-editing query strings, open the builder, choose the region/chapter/version you want staff to start from, expand **Reviewer setup**, add your review email and optional review form URL, then click **Copy staff link**. That copied link carries the review destination plus the current starting context.

Use **Open review form** as the primary return path when `review_url` is configured. It copies the submission JSON, opens the form, and appends only lightweight context such as `wrapped_scope`, `wrapped_slug`, `wrapped_variant`, and `wrapped_preview`. Do not rely on the builder to put the full submission JSON in the URL; those packets can be too long for reliable browser/server handling. Configure the Gravity Forms JSON textarea as `wrapped_submission` for pasted JSON and exported entries. Use **Open email draft** as the fallback when staff are sending JSON straight back to the maintainer; it copies the submission JSON first, then opens a short email note. The draft usually includes the submission JSON automatically; if the edit packet is too large for a reliable email link, staff should paste the copied JSON before sending or use **Download submission**. To make email fallback easier for a pilot group, include `review_email`, for example `builder.html?review_email=wrapped-review@example.org`. Use **Copy submission** when staff will paste the JSON into Slack, Teams, or a Gravity Form review field. Use **Download submission** when attaching a file is easier.

The builder blocks no-change review packets and packets without submitter name/email. Staff should make at least one copy, metric, CTA, logo, palette, hide/show, or custom-screen change before sending a submission back.
...
If several staff send JSON back, put the files in an ignored local `staff-submissions/` folder and run `review-builder-submissions.js` first. It validates every file without writing to the deployable config and prints a quick valid/invalid summary with submitter details. Files can be direct builder submission JSON, Gravity Forms entry JSON with the builder packet stored in `wrapped_submission`, or a single exported JSON array of entries. Each valid review item prints a ready-to-run **Dry run:** command. If the review label is an array entry such as `export.json[2]`, that command includes `--entry 2`. The dry run validates the packet and prints the submitter, reviewer note, preview URL, and change summary without writing to the deployable config. Then run the real merge, the normal QA gate, and commit/push the updated config.
...
Use the widget prefill to avoid asking the person to repeat context they already came from. The form can receive `wrapped_chapter`, `wrapped_chapter_slug`, `wrapped_region`, `wrapped_scope`, `wrapped_slug`, `wrapped_name`, `wrapped_variant`, `wrapped_year`, and `wrapped_url` when those hidden fields exist in Gravity Forms.
